...
c_Confirmed <- cdat |>
  ...
  pivot_longer(
    cols = BaselineConfirmed:PostConfirmed,
    names_to = "Wave",
    values_to = "Confirmed"
  ) |>
  mutate(
...
c_TotalCPS <- cdat |>
  ...
  pivot_longer(
    cols = BaselineTotalCPS:PostTotalCPS,
    names_to = "Wave",
    values_to = "TotalCPS"
  ) |>
  mutate(
...
c_FC <- cdat |>
  ...
  pivot_longer(
    cols = BaselineFCEntries:PostFCEntries,
    names_to = "Wave",
    values_to = "FC"
  ) |>
  mutate(
...
c_Prev <- cdat |>
  ...
  pivot_longer(
    cols = BaselinePreventive:PostPreventive,
    names_to = "Wave",
    values_to = "Prev"
  ) |>
  mutate(
...
## coerce counties down to three values
c_long <- c_long |>
  mutate(
    COUNTY = case_when(
