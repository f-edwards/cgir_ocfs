...
c_long_imp |> 
  ...
  summarize(TotalCPS = sum(TotalCPS, na.rm = T),
            Confirmed = sum(Confirmed, na.rm = T),
            FC = sum(FC, na.rm = T),
            Prev = sum(Prev, na.rm = T)) |> 
  group_by(GROUP, Wave, .imp) |> 
...
c_long_imp |>
  group_by(GROUP, Wave, .imp, ID) |>
  summarize(TotalCPS = sum(TotalCPS, na.rm = T)>0,
